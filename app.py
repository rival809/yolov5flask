...
model_dir = os.path.join(app.instance_path, 'model')
...
def detect():
    ...
    print(video)
    # subprocess.run("ls")
    subprocess.run(['python', 'detect.py', '--weights',os.path.join(model_dir, "best.pt") ,'--source', os.path.join(uploads_dir, secure_filename(video.filename))], shell=True)
